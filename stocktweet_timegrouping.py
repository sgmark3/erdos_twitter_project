...
def aggregate_time_delayed_stock_data(df_stock, df_tweet, DT1, DT2):
    ...
    for i in df_tweet.index:
        ...
        df_stocks_stocks = df_stocks[df_stocks['time_stamp']>= date_and_time]
        df_stocks_stocks['delta'] = df_stocks_stocks['time_stamp'].copy().apply(lambda x: (x-date_and_time).total_seconds())
        try:
